...
export function ContextHero({
  ...
  return (
    <div
      className={cn(
        'relative isolate overflow-hidden rounded-2xl bg-slate-950',
        className
      )}
    >
        ...
        {imagePath && !imageError ? (
          <>
            <img
              ...
              loading="lazy"
            />
            {!imageLoaded && <div className="absolute inset-0 bg-slate-800" />}
          </>
        ) : (
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-slate-950/90 via-slate-900/65 to-slate-950/35" />
      ...
      <div className="relative flex min-h-[320px] flex-col justify-end p-7 sm:p-10 lg:min-h-[380px] lg:p-12">
        <div className="max-w-3xl">
          ...
          <p className="mt-3 text-base font-medium text-slate-200 sm:text-lg lg:text-xl">
            {subtext}
          </p>
            ...
            <div className="mt-6 flex flex-wrap gap-2.5">
              {badges.map((b) =>
                b.href ? (
                  ...
                  <span
                    key={b.label}
                    className="rounded-full bg-white/10 px-4 py-2 text-xs font-bold uppercase tracking-wide text-slate-100"
                  >
                    {b.label}
                  </span>
                )
          ...
          <div className="mt-7 flex flex-wrap items-center gap-3">
            {primaryAction && (
              <Link
                ...
                className="inline-flex items-center gap-2 rounded-lg bg-white px-6 py-3 text-sm font-bold uppercase tracking-wide text-slate-900 transition-colors hover:bg-slate-100"
              >
            ...
            {secondaryActions?.map((a) => (
              ...
              >
                {a.icon === 'travel' ? <MapPin className="h-5 w-5 text-slate-200" /> : <Calendar className="h-5 w-5 text-slate-200" />}
                {a.label}
              </Link>
            ))}
          </div>
