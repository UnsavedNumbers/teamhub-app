    ...
    <div className="bg-background-light dark:bg-background-dark text-[#0d141b] dark:text-slate-50 min-h-screen flex flex-col">
      ...
      <header className="flex items-center justify-between px-10 py-6 border-b border-solid border-[#e7edf3] dark:border-slate-800 bg-white dark:bg-background-dark">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-black uppercase tracking-tighter">TeamHub</h2>
        </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-start py-12 px-6">
        ...
        <div className="max-w-[1200px] w-full text-center mb-10">
          ...
          <p className="text-[#4c739a] text-xl font-medium mt-4 max-w-2xl mx-auto uppercase tracking-widest">
            The Blueprint for your organization is ready for deployment.
          </p>
        </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl p-8 border border-[#e7edf3] dark:border-slate-800 flex flex-col md:flex-row items-center justify-between gap-8 shadow-sm">
            <div className="flex-1 text-center md:text-left">
              <div className="flex items-center justify-center md:justify-start gap-3 mb-2">
                ...
                <h2 className="text-2xl font-black uppercase tracking-tight">Access Limited</h2>
              </div>
              <p className="text-[#4c739a] text-lg">
                Your trial has expired. {canUpgrade ? 'Upgrade to restore full access to your athletic organization\'s dashboard and data-driven insights.' : 'Contact your organization administrator to upgrade and restore access.'}
              </p>
            </div>
            {canUpgrade && (
              <div className="flex flex-col items-center gap-2">
                <button
                  onClick={() => {
                    const firstPlan = planCards[0]
                    if (firstPlan) handleSelect(firstPlan.id)
                  }}
                  disabled={!!loadingPlan}
                  className="bg-primary text-white font-black text-xl px-12 py-5 rounded-lg shadow-[0_8px_0_0_#0d5bb1] hover:translate-y-[2px] hover:shadow-[0_6px_0_0_#0d5bb1] active:translate-y-[8px] active:shadow-none transition-all uppercase tracking-tight disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loadingPlan ? t('common.loading') : 'Upgrade Now'}
                </button>
                <p className="text-[10px] text-[#4c739a] font-bold uppercase mt-2">v1.0.4 Athletic Engine</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border border-[#cfdbe7] dark:border-slate-700 rounded-xl overflow-hidden">
            ...
            <div className="bg-[#f8f9fb] dark:bg-slate-950 p-10 flex flex-col gap-8 border-r border-[#cfdbe7] dark:border-slate-700">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-2xl font-black uppercase tracking-tight text-[#4c739a]">Restricted</h3>
              </div>
              <ul className="space-y-6">
                <li className="flex flex-col gap-1 border-b border-dashed border-[#cfdbe7] pb-4">
                  <p className="text-[#4c739a] text-xs font-black uppercase tracking-wider">Analytics State</p>
                  <p className="text-[#0d141b] dark:text-slate-300 text-lg font-bold">Locked Dashboard</p>
                </li>
                <li className="flex flex-col gap-1 border-b border-dashed border-[#cfdbe7] pb-4">
                  <p className="text-[#4c739a] text-xs font-black uppercase tracking-wider">Roster Capacity</p>
                  <p className="text-[#0d141b] dark:text-slate-300 text-lg font-bold">5 Athlete Limit</p>
                </li>
                <li className="flex flex-col gap-1 border-b border-dashed border-[#cfdbe7] pb-4">
                  <p className="text-[#4c739a] text-xs font-black uppercase tracking-wider">Data Modality</p>
                  <p className="text-[#0d141b] dark:text-slate-300 text-lg font-bold">Read-Only Viewer</p>
                </li>
              </ul>
            </div>
            {/* Restored Column */}
            <div className="bg-white dark:bg-slate-900 p-10 flex flex-col gap-8">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-2xl font-black uppercase tracking-tight text-primary">Restored</h3>
              </div>
              <ul className="space-y-6">
                <li className="flex flex-col gap-1 border-b border-dashed border-primary/20 pb-4">
                  <p className="text-primary text-xs font-black uppercase tracking-wider">Elite Intel</p>
                  <p className="text-[#0d141b] dark:text-white text-lg font-bold">Advanced Analytics Suite</p>
                </li>
                <li className="flex flex-col gap-1 border-b border-dashed border-primary/20 pb-4">
                  <p className="text-primary text-xs font-black uppercase tracking-wider">Scale Potential</p>
                  <p className="text-[#0d141b] dark:text-white text-lg font-bold">Unlimited Athlete Rosters</p>
                </li>
                <li className="flex flex-col gap-1 border-b border-dashed border-primary/20 pb-4">
                  <p className="text-primary text-xs font-black uppercase tracking-wider">Strategic Output</p>
                  <p className="text-[#0d141b] dark:text-white text-lg font-bold">Full Data & Export Control</p>
                </li>
              </ul>
            </div>
          </div>
        ...
        <div className="max-w-[960px] w-full text-center pb-20">
          <h3 className="text-xs font-black uppercase tracking-[0.3em] text-[#4c739a] mb-8">Why Organizations Upgrade</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="p-6 border border-[#e7edf3] dark:border-slate-800 rounded-lg bg-white dark:bg-slate-900 flex flex-col items-center justify-center gap-2">
              ...
              <p className="font-black uppercase tracking-tighter text-sm">Elite Performance</p>
            </div>
            <div className="p-6 border border-[#e7edf3] dark:border-slate-800 rounded-lg bg-white dark:bg-slate-900 flex flex-col items-center justify-center gap-2">
              <span className="material-symbols-outlined text-primary text-4xl">hub</span>
              <p className="font-black uppercase tracking-tighter text-sm">Centralized Data</p>
            </div>
            <div className="p-6 border border-[#e7edf3] dark:border-slate-800 rounded-lg bg-white dark:bg-slate-900 flex flex-col items-center justify-center gap-2">
              <span className="material-symbols-outlined text-primary text-4xl">verified_user</span>
              <p className="font-black uppercase tracking-tighter text-sm">Total Control</p>
            </div>
          </div>
        </div>
      </main>
      ...
      <footer className="mt-auto py-10 px-10 border-t border-[#e7edf3] dark:border-slate-800 bg-white dark:bg-slate-950">
        <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-[10px] font-black uppercase tracking-[0.2em] text-[#4c739a]">
            TeamHub © 2024. All systems operational.
          </div>
