  ...
  const columns: ColumnConfig<TravelPlan>[] = [
    ...
    { id: 'actions', label: 'Actions', align: 'right', render: (row) => (
      <div className="pa-flex pa-gap-2 pa-justify-end">
        ...
        {row.status !== 'published' && <Button variant="secondary" onClick={(e) => { e.stopPropagation(); handlePublish(row.id) }}>Publish</Button>}
        {row.status !== 'cancelled' && <Button variant="secondary" onClick={(e) => { e.stopPropagation(); handleCancel(row.id) }}>Cancel</Button>}
      </div>
    )}
  ]
