-- Fix get_fan_calendar - use correct enum value 'paid' instead of 'completed'
-- ticket_order_status enum: 'pending_payment', 'paid', 'refunded', 'cancelled'
...
BEGIN
  ...
  user_ticketed_event_ids AS (
    -- Get ticketed_event_ids from user's paid ticket orders
    -- ticket_orders columns: id, org_id, ticketed_event_id, purchaser_user_id, status, ...
    -- ticket_order_status enum: 'pending_payment', 'paid', 'refunded', 'cancelled'
  ...
  regular_events AS (
    ...
    -- event_visibility enum: 'public', 'unlisted', 'members', 'ticket_holders', 'private'
    SELECT
      e.id,
      e.title,
      e.start_time,
  ...
  ticketed_events_query AS (
    -- ticketed_events columns: id, org_id, title, description, starts_at, ends_at, timezone, venue_name, ...
    SELECT
      te.id,
      te.title as title,
...
IS 'Returns calendar events for fans. Regular events must be public and from followed orgs or bookmarked. Ticketed events shown if user has paid tickets.';
