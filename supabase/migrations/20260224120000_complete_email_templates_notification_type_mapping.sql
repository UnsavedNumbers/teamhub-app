-- Complete mapping of all email templates to notification types
-- This migration updates any templates that weren't linked in the previous migration
-- and ensures all templates have proper notification_type_id values
...
CREATE OR REPLACE FUNCTION map_job_type_to_notification_type_complete(job_type TEXT)
RETURNS TEXT AS $$
BEGIN
...
-- Step 1: Drop the unique index temporarily to allow updates
DROP INDEX IF EXISTS idx_email_templates_one_active_per_type;

-- Step 2: Identify templates that will be linked and deactivate duplicates BEFORE linking
-- Use a CTE to find which templates will map to the same notification_type_id
WITH templates_to_link AS (
  SELECT 
    et.id,
    et.is_active,
    et.updated_at,
    et.created_at,
    nt.id AS new_notification_type_id,
    ROW_NUMBER() OVER (
      PARTITION BY nt.id 
      ORDER BY et.is_active DESC, et.updated_at DESC, et.created_at DESC
    ) AS rn
  FROM public.email_templates et
  CROSS JOIN public.notification_types nt
  WHERE et.notification_type_id IS NULL
    AND nt.key = map_job_type_to_notification_type_complete(et.type::TEXT)
    AND map_job_type_to_notification_type_complete(et.type::TEXT) IS NOT NULL
)
-- Deactivate all but the first (most recent) template for each notification_type_id
UPDATE public.email_templates
SET is_active = false
WHERE id IN (
  SELECT id FROM templates_to_link WHERE rn > 1 AND is_active = true
);

-- Step 3: Now update notification_type_id (duplicates are already deactivated)
...
DROP FUNCTION IF EXISTS map_job_type_to_notification_type_complete(TEXT);

-- Step 4: Final cleanup - ensure only one active template per notification_type_id
-- This handles any edge cases where templates were already linked
UPDATE public.email_templates et
SET is_active = false
...
CREATE UNIQUE INDEX idx_email_templates_one_active_per_type
