-- Fix get_fan_calendar to properly JOIN events through teams table
-- The events table has team_id, not org_id directly
-- The org_id must be retrieved via teams.org_id
...
BEGIN
  ...
  regular_events AS (
    ...
    WHERE
      ...
      -- And user must follow org OR have bookmarked OR have tickets
      AND (
  ...
  ticketed_events_query AS (
    ...
    SELECT
      ...
      te.org_id,
      'public'::text as visibility, -- Ticketed events are public if user has tickets
...
COMMENT ON FUNCTION public.get_fan_calendar(TIMESTAMPTZ, TIMESTAMPTZ, UUID[], VARCHAR[]) 
