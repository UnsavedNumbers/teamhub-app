-- Fix get_fan_calendar - correct all column references
-- ticket_orders has purchaser_user_id not user_id
-- ticketed_events may not have venue_id in all cases
...
BEGIN
  ...
  WITH followed_orgs AS (
    -- fan_org_follows columns: id, user_id, org_id, source, created_at
    SELECT org_id FROM public.fan_org_follows WHERE user_id = v_user_id
  ),
  bookmarked_events AS (
    -- fan_event_bookmarks columns: id, user_id, event_id, created_at
    SELECT event_id FROM public.fan_event_bookmarks WHERE user_id = v_user_id
  ),
  user_ticketed_event_ids AS (
    -- Get ticketed_event_ids from user's ticket orders
    -- ticket_orders columns: id, org_id, ticketed_event_id, purchaser_user_id, ...
    SELECT DISTINCT tor.ticketed_event_id
    FROM public.ticket_orders tor
    WHERE tor.purchaser_user_id = v_user_id
      AND tor.status IN ('completed', 'paid')
  ),
  regular_events AS (
    -- events columns: id, team_id, season_id, title, type, start_time, end_time, location, notes, visibility, timezone, ...
    -- teams columns: id, org_id, name, ...
    -- organizations columns: id, name, slug, timezone, ...
    SELECT
      e.id,
      e.title,
      e.start_time,
  ...
  ticketed_events_query AS (
    -- ticketed_events columns: id, org_id, team_id, event_id, event_type, title (as name), description, 
    --   starts_at, ends_at, timezone, venue_name, venue_address_line1, ...
    -- Note: venue_id may exist from migration 20260405000010
    SELECT
      te.id,
      te.title as title,
      ...
      COALESCE(te.timezone, org.timezone, 'America/New_York') as timezone,
      te.org_id,
...
IS 'Returns calendar events for fans. Regular events must be public and from followed orgs or bookmarked. Ticketed events shown if user purchased tickets.';
