-- Migration: Update get_feature_gate for Add-On Entitlements
-- Description: Updates get_feature_gate() function to check org_addon_entitlements
--              after tier assignment checks but before final denial
-- Date: 2026-04-22

BEGIN;

-- ============================================================================
-- STEP 1: Update get_feature_gate function to include add-on entitlement check
-- ============================================================================
...
BEGIN
  ...
  IF v_license_tier_id IS NOT NULL THEN
    ...
    IF v_tier_exists IS NULL THEN
      v_license_tier_id := NULL; -- Tier was archived or doesn't exist
    END IF;
  END IF;
  ...
  -- =========================================================================
  -- CHECK ADD-ON ENTITLEMENT (NEW)
  -- If feature is not included in tier, check if org has purchased it as add-on
  -- =========================================================================
  
  IF p_org_id IS NOT NULL THEN
    SELECT status INTO v_addon_status
    FROM public.org_addon_entitlements
    WHERE org_id = p_org_id
      AND feature_key = p_feature_key
      AND status = 'active'
    LIMIT 1;

    IF v_addon_status = 'active' THEN
      -- Add-on entitlement is active, grant access
      RETURN jsonb_build_object(
        'allowed', TRUE,
        'gate_action', NULL,
        'reason_code', 'addon_entitlement',
        'feature_key', p_feature_key
      );
    END IF;
  END IF;

  -- No tier assignment, no parent inheritance, and no add-on entitlement
  -- Deny access (unless platform admin)
  IF v_is_platform_admin THEN
    RETURN jsonb_build_object(
      'allowed', TRUE,
      'gate_action', NULL,
...
system features, overrides (with parent check for children), tier assignments, add-on entitlements, and roles.
...
Add-on entitlements: checked after tier assignment; if active add-on exists, grant access.
