-- ============================================================================
-- UPDATE get_feature_gate FOR PARENT/CHILD INHERITANCE
-- ============================================================================
-- This migration updates get_feature_gate to implement child feature inheritance:
-- 1. Child features check their own tier assignment first
-- 2. If child is explicitly denied → deny immediately
-- 3. If child is explicitly allowed OR has no assignment → check parent
-- 4. If parent is denied → deny child (parent gate overrides child allowance)
-- 5. If parent is allowed → allow child (inherit parent access)
-- 
-- Override behavior: Parent denial always blocks children regardless of child overrides
-- ============================================================================
...
BEGIN
  ...
  -- =========================================================================
  -- HELPER FUNCTION: Check if parent feature is allowed
  -- =========================================================================
  -- This logic is reused in override checks and tier assignment checks
  -- Returns TRUE if parent is allowed, FALSE if denied, NULL if not a child
  -- =========================================================================
  -- Note: This is inline logic, not a separate function, to avoid recursion issues
...
-- Clear feature gate cache when parent_feature_key is updated
-- ============================================================================
CREATE OR REPLACE FUNCTION invalidate_feature_gate_cache_on_parent_change()
RETURNS TRIGGER AS $$
BEGIN
  -- When parent_feature_key changes, we need to invalidate cache for:
  -- 1. This feature (child)
  -- 2. All other children of the old parent (if changed)
  -- 3. All other children of the new parent (if changed)
  
  -- Note: Actual cache invalidation happens in application layer
  -- This trigger fires on parent_feature_key changes
  -- Application should listen via Supabase realtime or call clearFeatureGateCache() after bulk operations
...
DROP TRIGGER IF EXISTS trigger_invalidate_cache_on_parent_change ON feature_entitlements;

CREATE TRIGGER trigger_invalidate_cache_on_parent_change
  AFTER UPDATE OF parent_feature_key ON feature_entitlements
  FOR EACH ROW
  WHEN (OLD.parent_feature_key IS DISTINCT FROM NEW.parent_feature_key)
  EXECUTE FUNCTION invalidate_feature_gate_cache_on_parent_change();
