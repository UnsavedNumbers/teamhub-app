-- One-time migration to rename tier keys from descriptive names to generic tier1/tier2
-- This updates all tier_key values in license_tiers table and related function references

-- Step 1: Update tier_key values in license_tiers table
-- 'basic' -> 'tier1'
-- 'power' -> 'tier2'
UPDATE public.license_tiers
SET tier_key = CASE 
  WHEN tier_key = 'basic' THEN 'tier1'
  WHEN tier_key = 'power' THEN 'tier2'
  ELSE tier_key  -- Keep other tier keys as-is (if any exist)
END,
updated_at = now()
WHERE tier_key IN ('basic', 'power');

-- Step 2: Update hardcoded tier_key references in get_org_photo_storage_limit_bytes()
-- This function has hardcoded 'basic' and 'power' checks that need updating
...
BEGIN
  ...
  RETURN CASE
    WHEN v_tier_key = 'tier2' THEN 20::BIGINT * 1024 * 1024 * 1024  -- Updated from 'power'
    WHEN v_tier_key = 'tier1' THEN 1::BIGINT * 1024 * 1024 * 1024  -- Updated from 'basic'
