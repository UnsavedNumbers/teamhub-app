-- Fix get_fan_calendar - remove references to non-existent event_id on tickets table
-- Tickets only link to ticketed_events via ticketed_event_id, not regular events
...
BEGIN
  ...
  user_ticketed_event_ids AS (
    -- Get ticketed_event_ids from user's tickets (via ticket_types)
    SELECT DISTINCT tt.ticketed_event_id
    FROM public.tickets t
    JOIN public.ticket_types tt ON tt.id = t.ticket_type_id
    WHERE t.org_id IN (SELECT org_id FROM followed_orgs)
       OR EXISTS (SELECT 1 FROM public.ticket_orders o WHERE o.id = t.order_id AND o.user_id = v_user_id)
  ),
  regular_events AS (
...
IS 'Returns calendar events for fans. Regular events must be public and from followed orgs or bookmarked. Ticketed events shown if user has tickets.';
